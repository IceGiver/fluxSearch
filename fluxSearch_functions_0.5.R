...
refLibPos_ls <- function (refInput) {
        ...
        ref_pos <- data.frame(mz2, rt2, Lipid, fattyAcid, lipidClass, lipidForm,
                              Theoretical_mz, Adduct, Charge, Grade_ls)
        ref_pos$mz2 <- as.numeric(ref_pos$mz2)
        ref_neg$rt2 <- as.numeric(ref_pos$rt2)
        return(ref_pos)
}
